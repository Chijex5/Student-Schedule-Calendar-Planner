...
export const GeneratedCalendar = ({
  ...
  const getDaysInMonth = (date: Date): (Date | null)[] => {
    ...
    for (let i = 1; i <= lastDay.getDate(); i++) days.push(new Date(date.getFullYear(), date.getMonth(), i));
    return days;
  };
  ...
  return <div>
      ...
      {!isMobile && <div className="grid grid-cols-7 mb-4">
        ...
        return <div key={index} className={`
              ...
              {date && <>
                  <div className="flex justify-between items-center">
                    ...
                    {isMobile && scheduleItem && (isExpanded ? <ChevronUp size={16} className="text-[#E0B0FF]" /> : <ChevronDown size={16} className="text-[#E0B0FF]" />)}
                  </div>

                  {(isExpanded || !isMobile) && scheduleItem && <div className="mt-2 space-y-1">
